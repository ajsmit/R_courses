...
A very clear example of environmental and species datasets is the Doubs River dataset. Please refer to [David Zelený's website for an explanation](https://www.davidzeleny.net/anadat-r/doku.php/en:data:doubs) of these data. These data demonstrate nicely how one of the basic drivers of biodiversity patterning---gradients---can be seen operating on in a real world case study. It offers neat insight also into the properties of species and environmental tables and the dissimilarity and distance atrices derived from them.
...
Next we see the table view of the environmental data. The names of the 11 environmental variables appear as column headers and there are 30 rows, one for each of the samples---the samples match that of the species data.
...
Although there are manny ways in which species can respond to their environment, one of the most striking responses can be seen along environmental gradients. We will explore this concept next as we discuss coenoclines and unimodal species distribution models.
...
In the above equation, this 'distance' is calculated between a pair of sites, ***a*** and ***b***, whose locations are marked by the coordinates `\(x\)`, `\(y\)`, and `\(z\)`---i.e. this is an example of 3-dimensional data (a space or volume, as opposed to 2D data situated on a `\(x\)`, `\(y\)` place). We might also call each coordinate a 'variable' such as pH or temperature (sometimes called a 'dimension').

Let's [download some example data](/BDC334/data/Euclidian_distance_demo_data_xyz.csv) and calculate the distance between every pair of sites named ***a*** to ***g***.

The 'raw' data representing `\(x\)`, `\(y\)` and `\(z\)` simensions look like this in MS Excel:
...
We can calculate Euclidian distance in Excel using some buil-in functions:
...
Please label the MS Excel and MS Word files as follows: **BDC334_<first_name>_<last_name>_Assignement_1.xlsx** and **BDC334_<first_name>_<last_name>_Assignement_1.docx** (the '<' and '>' must be ommitted as they are used in the example as field indicators only).
