...
**This is a course about community ecology across different spatial
\[and temporal\] scales.** Community ecology underpins the vast fields
of biodiversity and biogeography and concerns spatial scales from
squares of meters to all of Earth. We can look at historical,
contemporary, and future processes that have been implicated in shaping
...
[Distances](/bdc334/chapters/01-introduction/#distances) in the
'Pair-wise matrices' section below, but the latter does not have
decimals:

![**Figure 1.** The content of a CSV file seen in VS Code.](../images/csv_file.png)
...
![**Figure 2.** The Doubs River species data in MS Excel.](../images/DoubsSpe_table.png)
...
![**Figure 3.** The Doubs River environmental data.](../images/DoubsEnv_table.png)
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#| fig-cap: "Figure 5. A coenoplane with smooth surface showing idealised species responses."
set.seed(10)
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#| fig-cap: "Figure 6. A coenoplane with 'smooth surface'raw' abundance data."
...
are symmetrical (or lower triangle) square matrices (i.e. *n* × *n*).
...
A frequently used **distance metric** is Euclidean **distance**.
Euclidean distance is the 'ordinary straight-line' distance between two
points in Euclidean space. Working with geographical coordinates over
small areas of Earth's surface, Euclidean distance is very similar (i.e.
...
units in Euclidean space. The implication is that on a map or on a
...
data](../data/Euclidian_distance_demo_data_xyz.csv) and calculate the
distance between every pair of sites named ***a*** to ***g***.

The 'raw' data representing *x*, *y* and *z* dimensions look like this
in MS Excel:

![**Figure 7.** Three-dimensional data at seven sites.](../images/Euclidian_xyz.png)

Or we can substitute *x*, *y* and *z* for environmental 'dimensions' and
we have this:

![**Figure 7.**'Three-dimensional' environmental data at seven sites.](../images/Euclidian_env.png)
...
We can calculate Euclidean distance in Excel using some built-in
functions:

![**Figure 8.** Calculating Euclidean distances in MS Excel.](../images/Euclidian_env_calc.png)
...
done between columns (variables), not rows as in the Euclidean distance
calculation, which compares the rows (sites). We do not need to
standardise as one would do for the calculation of Euclidean distances.
...
![**Figure 9.** Calculating pairwise correlations in MS Excel.](../images/Correlation_env_calc.png)
